...
{
    public class RiivolutionUniversalISOBuilderLin
    {
        public void Main(string[] args)
        {
            ...
            if (args.Length > 0) // Arguments
            {
                if (args[0] == "-h" || args[0] == "--help" || args[0] == "/?")
                {
                    ...
                    Console.WriteLine(
                        "Usage: UniversalISOBuilder.exe <ISO Path> <Riivolution XML file path> <Output ISO/WBFS path> [options]");
                    Console.WriteLine("       UniversalISOBuilder.exe [options]");
                    Console.WriteLine("       UniversalISOBuilder.exe");
                    Console.WriteLine(
                        "       In the 2nd and 3rd cases, you will be asked for the file paths.\r\n\r\n"); // Thanks to Mullkaw for correcting my weird-sounding english! ^^
                    Console.WriteLine(
                        "Options: --silent                  -> Prevents from displaying any console outputs apart from the necessary ones");
                    Console.WriteLine(
                        "         --always-single-choice    -> Enables any option that only has one choice");
                    Console.WriteLine(
                        "         --never-single-choice     -> Disable any option that only has one choice");
                    Console.WriteLine(
                        "         --title-id <TitleID>      -> Changes the TitleID of the output rom; Use periods in place of characters in the TitleID that shouldn't be replaced.");
                    Console.WriteLine(
                        "         --game-name <Game name>   -> Changes the game name of the output rom; If the name you want to set contains spaces, put it between quotes.");
                    Console.WriteLine(
                        "         --keep-extracted-iso      -> Prevents the temporary extracted patched rom folder from being deleted at the end of the build process");
                    Console.WriteLine(
                ...
                if (args.Contains(
                        "--skip-all-done")) // Not putting this in the help page, it's just for my personal use
                {
                    skipAllDone = true;
                ...
                if (args.Contains("--title-id"))
                {
                    ...
                    if (newTitleID.Length != 6)
                    {
                        Console.WriteLine("Invalid TitleID " + newTitleID + ".");
                        return;
                    }
                ...
                if (isThereDefinedPaths) // Paths are already defined? Directly do the stuff.
                {
                    ...
                    doStuff(ISOPath, xmlPath, outPath, singleChoice, newTitleID, newGameName, isSilent, deleteISO,
                        ignoreLevel, skipAllDone);
                    return;
                }
            }
                        ...
                        using (OpenFileDialog dialog2 = new OpenFileDialog())
                        {
                            ...
                            if (dialog2.ShowDialog(parent: null) == DialogResult.Ok)
                            {
                                ...
                                if (textDialog.ShowDialog(parent: null) == DialogResult.Ok)
                                {
                                    ...
                                    doStuff(dialog.FileName, dialog2.FileName, outPath, singleChoice, newTitleID,
                                        newGameName, isSilent, deleteISO, ignoreLevel, skipAllDone);
                                }
                                else
                                {
        ...
        public void doStuff(string ISOPath, string xmlPath, string outPath, string singleChoice, string newTitleID,
            string newGameName, bool isSilent, bool deleteISO, int ignoreLevel, bool skipAllDone)
        {
            string currentShell = Environment.GetEnvironmentVariable("SHELL");
            ;
            Random random = new Random();
            const string chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
            string extDir = Path.GetDirectoryName(AppDomain.CurrentDomain.BaseDirectory + "patchedRom/" +
                                                  Path.GetFileNameWithoutExtension(xmlPath) + "-" +
                                                  new string(Enumerable.Repeat(chars, 6)
            ...
            {
                Console.WriteLine("Warning: This riivolution patch only applied to the game that has the TitleID " +
                                  disc.gameFilter.game + ".\r\nYours uses the TitleID " + gameID +
                                  " and therefore cannot be patched.");
                if (ignoreLevel < 1)
                {
                    Console.WriteLine(
                        "If you really know what you're doing and want to ignore this, try to run this tool again with the \"--ignore-warnings\" option.");
                    return;
                }
            }
            ...
            {
                Console.WriteLine(
                    "No custom Title ID were specified, therefore your mod will use the same save slot as the game you're modding, which could cause issues with some mods!");
            }
            ...
            runCommand("tools/wit",
                "extract -s " + ISOPath + " -1 -n " + titleID + " . \"" + extDir + "\" --psel=DATA -ovv", isSilent);

            Console.WriteLine("ISO Extracted.");

            List<Patch> patches = new List<Patch>();

            if (!(disc.sections.Count == 1 && disc.sections[0].options.Count == 1 &&
                  disc.sections[0].options[0].choices.Count == 1))
            {
                foreach (Section section in disc.sections)
                {
                    ...
                    foreach (Option option in section.options)
                    {
                        ...
                        else
                        {
                            ...
                            if (singleChoice == "Ask")
                            {
                                Console.Write("   Only one choice found: " + option.choices[0].name +
                                              " - Use it? (Yes/No): ");
            ...
            copy.StartInfo.FileName = currentShell;
            copy.StartInfo.UseShellExecute = false;
            copy.StartInfo.RedirectStandardOutput = true;
            copy.StartInfo.WorkingDirectory =
                Path.GetDirectoryName(EtoEnvironment.GetFolderPath(EtoSpecialFolder.EntryExecutable));
            foreach (Patch patch in patches)
            {
                ...
                foreach (File filePatch in patch.filePatches)
                {
                    ...
                    if ((filePatch.disc == null || filePatch.disc == "")
                            ? true
                            : Directory.Exists(
                                extPath)) // Avoid running useless copy commands for folders that doesn't exist AND don't have the "created" flag enabled (this is used for region-specific folders in NewerSMBW, for example)
                    {
                        if (filePatch.disc != "" && filePatch.disc != null)
                        {
                            if (!isSilent) Console.WriteLine("Copying " + patch.root + "/" + filePatch.external);

                            copy.StartInfo.Arguments =
                                "-c \"rsync -a --ignore-errors " + file + "\" \"" + extPath + "\"";
                        }
                        else if (Directory.Exists(file))
                        {
                            if (!isSilent)
                                Console.WriteLine("Searching manually for file named " + Path.GetFileName(file));
                            ...
                            {
                                Console.WriteLine("");
                            } // Just for good-looking purposes.
                        ...
                        if (!isSilent)
                        {
                            Console.WriteLine(copy.StandardOutput.ReadToEnd());
                        }
                        else
                        {
                ...
                foreach (Folder folderPatch in patch.folderPatches)
                {
                    ...
                    if ((folderPatch.disc == null || folderPatch.disc == "")
                            ? true
                            : Directory.Exists(
                                extPath)) // Avoid running useless copy commands for folders that doesn't exist AND don't have the "created" flag enabled (this is used for region-specific folders in NewerSMBW, for example)
                    {
                        if (folderPatch.disc != "" && folderPatch.disc != null)
                        {
                            ...
                            copy.StartInfo.Arguments = "-c \"rsync --ignore-errors " + path + " " + extPath + "\"" +
                                                       recursive + " /C /I /Y";
                        }
                ...
                foreach (Memory memoryPatch in patch.memoryPatches)
                {
                    ...
                    {
                        if (!isSilent)
                            Console.WriteLine("Trying to patch " + memoryPatch.value.AsString() + " at 0x" +
                                              memoryPatch.offset.ToString("X8") + "...");

                        bool success = dp.doMemoryPatch(memoryPatch.offset, memoryPatch.value.ToArray(),
                            memoryPatch.original.ToArray());

                        if (!isSilent && success) Console.WriteLine("Success.\r\n");
                        else if (!isSilent && !success)
                            Console.WriteLine(
                                "Couldn't apply this patch (This isn't necessarely an error, this is usually meant for different regions support).\r\n");
                    }
                    else
                    {
                        ...
                        {
                            if (!isSilent)
                                Console.WriteLine("Binary file " + memoryPatch.valueFile + " found, patching...");

                            bool success = dp.doMemoryPatch(memoryPatch.offset,
                                System.IO.File.ReadAllBytes(memFilePath), memoryPatch.original.ToArray());

                            if (!isSilent && success)
                                Console.WriteLine("Binary file " + memoryPatch.valueFile +
                                                  " patched successfully.\r\n");
                            else if (!isSilent && !success)
                                Console.WriteLine("Binary file " + memoryPatch.valueFile + " coudln't be patched.\r\n");
                        }
                    }
                }
            }
            ...
            if (!skipAllDone)
            {
                ...
                {
                    MessageBox.Show(
                        "WARNING: This mod uses savegame patches, which can't be integrated into an ISO/WBFS.\r\n" +
                        ...
                        "However, some mods use this kind of patch to have the default save file changed." /*" If this is your case, ask for support on discord at Asu-chan#2929"*/,
                        "Memory Patches Warning", MessageBoxButtons.OK, MessageBoxType.Warning);
                }

                if (disc.badMemPatches)
                {
                    MessageBox.Show(
                        "WARNING: Due to some memory patches being applied under 0x80004000, your game may not work on USB Loaders.\r\n\r\n" +
                        ...
                        "If you can, ask your mod's maker to put their code hacks in a different place.",
                        "Memory Patches Warning", MessageBoxButtons.OK, MessageBoxType.Warning);
                }
            }
            ...
            runCommand("tools/wit",
                "copy \"" + extDir + "\" \"" + outPath + "\" -ovv --disc-id=" + titleID + " --tt-id=" + gameID +
                " --name \"" + gameName + "\"", isSilent);

            if (deleteISO)
            {
                if (!isSilent)
                {
                    Console.WriteLine("Removing extracted patched rom directory...");
                }
