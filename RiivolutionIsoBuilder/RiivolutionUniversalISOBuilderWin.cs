...
public class RiivolutionUniversalISOBuilderWin
{
            ...
            using (OpenFileDialog dialog = new OpenFileDialog())
            {
                ...
                if (dialog.ShowDialog(null) == DialogResult.Ok)
                {
                    ...
                    using (OpenFileDialog dialog2 = new OpenFileDialog())
                    {
                        ...
                        if (dialog2.ShowDialog(null) == DialogResult.Ok)
                        {
                            ...
                            if (textDialog.ShowDialog(null) == DialogResult.Ok)
                            {
                                System.IO.StreamWriter sw =
                                    new System.IO.StreamWriter(System.IO.File.OpenWrite(textDialog.FileName));
        ...
        string newGameName, bool isSilent, bool deleteISO, int ignoreLevel, bool skipAllDone)
    {
        ...
        string extDir = Path.GetDirectoryName(AppDomain.CurrentDomain.BaseDirectory) + "\\" +
                        Path.GetFileNameWithoutExtension(xmlPath) + "-" + new string(Enumerable.Repeat(chars, 6)
        ...
        copy.StartInfo.WorkingDirectory = Path.GetDirectoryName(AppDomain.CurrentDomain.BaseDirectory);
                            ...
                            extPath)) // Avoid running useless copy commands for folders that doesn't exist AND don't have the "created" flag enabled (this is used for region-specific folders in NewerSMBW, for example)
                {
                    if (folderPatch.disc != "" && folderPatch.disc != null)
                    {
                        ...
                        copy.StartInfo.Arguments =
                            "/C xcopy \"" + path + "\" \"" + extPath + "\"" + recursive + " /C /I /Y";
                    }
            ...
            foreach (Memory memoryPatch in patch.memoryPatches)
            {
                ...
                else
                {
                    ...
                    if (System.IO.File.Exists(memFilePath))
                    {
                        ...
                        bool success = dp.doMemoryPatch(memoryPatch.offset, System.IO.File.ReadAllBytes(memFilePath),
                            memoryPatch.original.ToArray());

                        if (!isSilent && success)
                            Console.WriteLine("Binary file " + memoryPatch.valueFile + " patched successfully.\r\n");
                        else if (!isSilent && !success)
    ...
    public void runCommand(string executable, string arguments, bool isSilent)
    {
        ...
        command.StartInfo.WorkingDirectory = Path.GetDirectoryName(AppDomain.CurrentDomain.BaseDirectory);
        command.Start();
